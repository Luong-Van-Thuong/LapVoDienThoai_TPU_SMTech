...
namespace TPU_Assembly.Class
{
    public class YoloInferenceEngine : IDisposable
    {
        ...
        {
            var (inputTensor, gain, padX, padY) = PreprocessImageLetterbox(image);
            ...
            using (var results = _session.Run(inputs))
            {
                ...
                return ParseOutputOptimized(outputTensor, gain, padX, padY);
            }
        }
        ...
        private (Tensor<float> tensor, float gain, float padX, float padY) PreprocessImageLetterbox(Bitmap image)
        {
            int w = image.Width;
            int h = image.Height;

            float gain = Math.Min((float)_inputWidth / w, (float)_inputHeight / h);
            int newW = (int)Math.Round(w * gain);
            int newH = (int)Math.Round(h * gain);

            float padX = (_inputWidth - newW) / 2f;
            float padY = (_inputHeight - newH) / 2f;

            var tensor = new DenseTensor<float>(new[] { 1, 3, _inputHeight, _inputWidth });

            using (Bitmap resized = new Bitmap(image, new Size(newW, newH)))
            using (Bitmap canvas = new Bitmap(_inputWidth, _inputHeight))
            using (Graphics g = Graphics.FromImage(canvas))
            {
                g.Clear(Color.FromArgb(114, 114, 114));
                g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.HighQualityBilinear;
                g.PixelOffsetMode = System.Drawing.Drawing2D.PixelOffsetMode.HighQuality;
                g.DrawImage(resized, padX, padY, newW, newH);

                BitmapData data = canvas.LockBits(new Rectangle(0, 0, _inputWidth, _inputHeight), ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);
                unsafe
                {
        ...
        private List<Detection> ParseOutputOptimized(Tensor<float> output, float gain, float padX, float padY)
        {
            ...
            Parallel.For(0, _numPredictions, i =>
            {
                ...
                {
                    float xCenter = outputData[0 * _numPredictions + i];
                    float yCenter = outputData[1 * _numPredictions + i];
                    float w = outputData[2 * _numPredictions + i];
                    float h = outputData[3 * _numPredictions + i];

                    float xMin = (xCenter - w / 2 - padX) / gain;
                    float yMin = (yCenter - h / 2 - padY) / gain;
                    float width = w / gain;
                    float height = h / gain;
